...
$EmpProfile = get_userdata($User_Id);

$loggedInUser = get_current_user_id();

$fetchEmp = get_user_meta($loggedInUser, 'selected_employees', true);
$Savedempoyees = explode(',', $fetchEmp);
if (in_array($User_Id, $Savedempoyees))
  ...
  {
  	wp_redirect(get_the_permalink(790));
  	exit;
  }
...
// $fullname = $first_name.' '.$last_name;
$email = $EmpProfile->user_email;
$userlink_name = $EmpProfile->user_nicename;
...
?>
<style type="text/css">
	li.list-display .action-dropdown.dropdown, #gapButton, #viewAllCase{display: none !important;}
</style>